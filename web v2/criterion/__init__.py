...
def collect_data(safety, tasks, job_type, placeholder):
    global capture_data
    capture_data = placeholder
    global actions_dict, safe_tasks, unsafe_tasks, for_human, safe_locations, unsafe_locations, actions,task_list
    ...
    task_list = dict(zip(keys,actions))

def first_criteria():
    global result, task_list
    robot_task = {}
...
def second_criteria(tank_height, human_height):
    global result,task_list
    colaborative = {}
    to_be_removed =[]
    for h,i in task_list.items():
...
def fourth_criteria(fcvalues): #"for each ofthe above task enter a list of values indicating 1 for collaborative and 0 for otherwise [1,2,, 2,]"
    global result,task_list
    to_be_removed = []
   
    for  vals,tasks in zip(fcvalues,task_list.items()):
        if vals == 1:
...
def fifth_criteria(time_human, time_robot,operating_cost):
    global result,task_list
    to_be_removed = []
    #for each of the task above enter time it takes for human to complete as a list
    #time_robot =st.text_input( "for each of the task above enter time it takes for robot to complete as a list
    #operating_cost =st.text_input("for each of  the task enter  Enter the level of operating cost 3 HIGH, 2 MID, 1 LOW
